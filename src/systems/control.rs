    ...
    fn run(&mut self, (
        ...
        for (controller) in (&controller_storage).join() {
            ...
            if controller.right_mouse_released {
                ...
                char_state.target = if let Some(target_entity) = entity_below_cursor {
                    if let Some(target_entity) = entity_below_cursor {
                    ...
                    Some(EntityTarget::Pos(dbg!(mouse_world_pos)))
                };
            }
            ...
            if controller.is_key_just_pressed(Scancode::Q) {
                let dir = CharacterControlSystem::determine_dir(&mouse_world_pos, &char_pos);
                ...
                let new_state = CharState::CastingSkill {
                    ...
                    can_move: false,
                    skill: Skills::Test(mouse_world_pos),
                };
...
impl CharacterControlSystem {
    pub fn picking_2d_3d(x2d: u16, y2d: u16, camera: &Camera, matrices: &RenderMatrices) -> Point2<f32> {
