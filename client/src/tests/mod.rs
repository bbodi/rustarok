...
use crate::grf::texture::DUMMY_TEXTURE_ID_FOR_TEST;
...
use rustarok_common::components::char::{EntityId, EntityTarget};
use specs::prelude::*;
...
fn setup_ecs_world<'a, 'b>() -> TestUtil<'a, 'b> {
    ...
    let ecs_dispatcher = register_server_systems();
...
impl<'a> OrderedEventAsserter<'a> {
    ...
    pub fn status_change(
        mut self,
        expected_char_id: EntityId<Local>,
    ...
    pub fn state_went_into_attacking(
        mut self,
        attacker_id: EntityId<Local>,
        attacked_id: EntityId<Local>,
    ) -> OrderedEventAsserter<'a> {
        if !self.search_event(|event| match event {
    ...
    pub fn basic_damage(
        mut self,
        expected_attacker: EntityId<Local>,
        expected_attacked: EntityId<Local>,
    ) -> OrderedEventAsserter<'a> {
        if !self.search_event(|event| match event {
...
impl<'a, 'b> TestUtil<'a, 'b> {
    ...
    pub fn create_char(&mut self, pos: Vec2, team: Team) -> EntityId {
        let char_id = EntityId::from(self.ecs_world.create_entity().build());
        {
    ...
    pub fn cast_skill_on_pos(&mut self, char_id: EntityId<Local>, skill: Skills, pos: Vec2) {
    ...
    pub fn cast_skill_on_self(&mut self, char_id: EntityId<Local>, skill: Skills) {
        let mut char_storage = self.ecs_world.write_storage::<CharacterStateComponent>();
        let char_state = char_storage.get_mut(char_id.into()).unwrap();
        NextActionApplierSystem::try_cast_skill(
            skill,
    ...
    pub fn set_char_target(&mut self, char_id: EntityId<Local>, target: EntityTarget) {
        let mut char_storage = self.ecs_world.write_storage::<CharacterStateComponent>();
        let char_state = char_storage.get_mut(char_id.into()).unwrap();
        char_state.target = Some(target);
    }

    pub fn assert_on_character(&self, char_id: EntityId) -> CharAsserter {
        CharAsserter {
