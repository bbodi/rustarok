...
class GroundRenderer(gl: WebGL2RenderingContext) {

    private val ground_gl_program = load_ground_shader(gl)
    private var ground_vertex_count: Int = 0
    ...
    fun render_ground(gl: WebGL2RenderingContext, command: RenderCommand.Ground3D) {
        ...
        gl.vertexAttribPointer(ground_gl_program.a_tile_color_coord,
                               2,
        ...
        gl.bindTexture(WebGLRenderingContext.TEXTURE_2D,
                       get_or_load_server_texture(command.server_texture_atlas_id,
        ...
        gl.bindTexture(WebGLRenderingContext.TEXTURE_2D,
                       get_or_load_server_texture(command.server_tile_color_texture_id,
                                                  WebGLRenderingContext.LINEAR).texture)
        gl.uniform1i(ground_gl_program.tile_color_texture, 1)

        gl.activeTexture(WebGLRenderingContext.TEXTURE2)
        gl.bindTexture(WebGLRenderingContext.TEXTURE_2D,
                       get_or_load_server_texture(command.server_lightmap_texture_id,
                                                  WebGLRenderingContext.LINEAR).texture)
    ...
    fun set_vertex_buffer(gl: WebGL2RenderingContext, raw: Uint8Array, vertex_count: Int) {
        ground_vertex_buffer = create_vertex_buffer(gl, raw)
        ground_vertex_count = vertex_count
    }

    private fun load_ground_shader(gl: WebGL2RenderingContext): GroundShader {
...
void main() {
    ...
    private data class GroundShader(val program: WebGLProgram,
